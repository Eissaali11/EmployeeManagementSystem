#!/usr/bin/env python3
"""
إنشاء وظيفة تصدير Excel محدثة بالحقول الصحيحة
"""

# وظيفة تصدير جديدة بالحقول الصحيحة
export_function = '''
@vehicles_bp.route('/export_all_excel')
@login_required
def export_all_vehicles_excel():
    """تصدير جميع بيانات المركبات إلى ملف Excel شامل - محدث"""
    try:
        import io
        import pandas as pd
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            ...
            for vehicle in vehicles:
                ...
                for handover in handovers:
                    handover_data.append({
                        ...
                        'اسم الموظف': handover.employee_name or '',
                        'التاريخ': handover.handover_date.strftime('%Y-%m-%d') if handover.handover_date else '',
                        'قراءة العداد': handover.odometer_reading or '',
            ...
            # ===== الورقة السادسة: الفحوصات الدورية =====
            inspection_data = []
            for vehicle in vehicles:
                ...
                for inspection in inspections:
                    inspection_data.append({
                        ...
                        'النتيجة': inspection.result or '',
                        'التكلفة': inspection.cost or 0,
                        'ملاحظات': inspection.notes or ''
                    })
            
            if inspection_data:
                df_inspection = pd.DataFrame(inspection_data)
                df_inspection.to_excel(writer, sheet_name='الفحوصات الدورية', index=False)
            
            # ===== الورقة السابعة: فحوصات السلامة =====
            safety_data = []
            for vehicle in vehicles:
                safety_checks = VehicleSafetyCheck.query.filter_by(vehicle_id=vehicle.id).all()
                for check in safety_checks:
                    safety_data.append({
                        'رقم اللوحة': vehicle.plate_number or '',
                        'تاريخ الفحص': check.check_date.strftime('%Y-%m-%d') if check.check_date else '',
                        'النتيجة': check.result or '',
                        'ملاحظات': check.notes or ''
                    })
            ...
            # ===== الورقة الثامنة: الحوادث =====
            accident_data = []
            for vehicle in vehicles:
                accidents = VehicleAccident.query.filter_by(vehicle_id=vehicle.id).all()
                for accident in accidents:
                    accident_data.append({
                        'رقم اللوحة': vehicle.plate_number or '',
                        'تاريخ الحادث': accident.accident_date.strftime('%Y-%m-%d') if accident.accident_date else '',
                        'الموقع': accident.location or '',
                        'الوصف': accident.description or '',
                        'التكلفة': accident.cost or 0,
                        'حالة التأمين': accident.insurance_status or '',
                        'ملاحظات': accident.notes or ''
                    })
            
            if accident_data:
                df_accident = pd.DataFrame(accident_data)
                df_accident.to_excel(writer, sheet_name='الحوادث', index=False)
...
print("تم إنشاء وظيفة التصدير المحدثة")