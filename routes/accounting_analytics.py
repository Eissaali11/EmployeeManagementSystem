...
def has_accounting_access():
    ...
    return current_user.role == UserRole.ADMIN or current_user.has_module_access(Module.ACCOUNTING)
...
def dashboard():
    ...
    try:
        ...
        for month in range(1, 13):
            # إيرادات الشهر
            month_revenue = db.session.query(func.sum(TransactionEntry.amount)).join(Transaction).filter(
                TransactionEntry.account_id.in_([acc.id for acc in revenue_accounts]),
                Transaction.fiscal_year_id == fiscal_year.id,
                TransactionEntry.entry_type == EntryType.CREDIT,
            ...
            # مصروفات الشهر
            month_expenses = db.session.query(func.sum(TransactionEntry.amount)).join(Transaction).filter(
                TransactionEntry.account_id.in_([acc.id for acc in expense_accounts]),
                Transaction.fiscal_year_id == fiscal_year.id,
                TransactionEntry.entry_type == EntryType.DEBIT,
                extract('month', Transaction.transaction_date) == month
            ).scalar() or 0
        ...
        # توزيع المصروفات
        salary_expenses = db.session.query(func.sum(TransactionEntry.amount)).join(Transaction).filter(
            Transaction.transaction_type == TransactionType.SALARY,
            Transaction.fiscal_year_id == fiscal_year.id,
            TransactionEntry.entry_type == EntryType.DEBIT
        ).scalar() or 0
        
        vehicle_expenses = db.session.query(func.sum(TransactionEntry.amount)).join(Transaction).filter(
        ...
        predicted_expenses = sum(expense_data[-3:]) / 3 if len(expense_data) >= 3 else 0
...
def generate_financial_predictions(revenue_data, expense_data):
    ...
    try:
        ...
        if len(revenue_data) >= 3:
            ...
            for i in range(3):
                ...
                future_predictions.append(max(0, predicted_value))  # تجنب القيم السالبة
    ...
    return revenue_data, revenue_data, [f"شهر {i}" for i in range(1, len(revenue_data) + 1)]
...
def financial_data_api():
    ...
    return jsonify({
        'status': 'success',
        'message': 'البيانات المالية متاحة'
    })