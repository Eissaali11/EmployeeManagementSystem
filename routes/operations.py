...
@operations_bp.route('/export-notifications-excel')
@login_required
def export_notifications_excel():
    """تصدير إشعارات العمليات إلى ملف Excel"""
    try:
        # التحقق من صلاحية المدير
        if current_user.role != 'admin':
            flash('غير مصرح لك بالوصول لهذه الصفحة', 'danger')
            return redirect(url_for('mobile.operations_dashboard'))
        
        # جلب جميع الإشعارات
        notifications = OperationNotification.query.order_by(OperationNotification.created_at.desc()).all()
        
        if not notifications:
            flash('لا توجد إشعارات للتصدير', 'warning')
            return redirect(request.referrer or url_for('mobile.operations_notifications'))
        
        # إعداد البيانات للتصدير
        notifications_data = []
        for notification in notifications:
            # جلب بيانات العملية المرتبطة إن وجدت
            operation_title = ""
            vehicle_plate = ""
            if notification.operation_id:
                operation = OperationRequest.query.get(notification.operation_id)
                if operation:
                    operation_title = operation.title or ""
                    if operation.vehicle_id:
                        vehicle = Vehicle.query.get(operation.vehicle_id)
                        if vehicle:
                            vehicle_plate = vehicle.plate_number or ""
            
            # تحديد نوع الإشعار
            notification_type_ar = {
                'system': 'إشعار نظام',
                'operation': 'إشعار عملية',
                'alert': 'تنبيه',
                'info': 'معلومات'
            }.get(notification.notification_type, notification.notification_type or '')
            ...
            priority_ar = {
                ...
                'normal': 'عادي',
                'low': 'منخفض'
            }.get(notification.priority, notification.priority or '')
            
            notifications_data.append({
                'رقم الإشعار': notification.id,
                'العنوان': notification.title or '',
                'الرسالة': notification.message or '',
                'نوع الإشعار': notification_type_ar,
                'الأولوية': priority_ar,
                'حالة القراءة': 'مقروء' if notification.is_read else 'غير مقروء',
                'عنوان العملية': operation_title,
                'رقم لوحة السيارة': vehicle_plate,
                'تاريخ الإنشاء': notification.created_at.strftime('%Y-%m-%d %H:%M:%S') if notification.created_at else '',
                'تاريخ القراءة': notification.read_at.strftime('%Y-%m-%d %H:%M:%S') if notification.read_at else ''
            })
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='إشعارات العمليات', index=False)
            
            # تنسيق الورقة
            worksheet = writer.sheets['إشعارات العمليات']
            
            # تحديد عرض الأعمدة
            column_widths = {
                'A': 15, 'B': 30, 'C': 40, 'D': 20, 'E': 15,
                'F': 20, 'G': 30, 'H': 20, 'I': 25, 'J': 25
            }
        ...
        # تحديد اسم الملف
        filename = f"notifications_operations_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
