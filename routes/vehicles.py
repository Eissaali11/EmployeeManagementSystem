...
# طرق إدارة فحوصات السلامة مع تسجيل العمليات
@vehicles_bp.route('/<int:vehicle_id>/safety_check_details/<int:check_id>')
@login_required
def view_safety_check_details(vehicle_id, check_id):
    """عرض تفاصيل فحص سلامة محدد"""
    vehicle = Vehicle.query.get_or_404(vehicle_id)
    safety_check = VehicleSafetyCheck.query.filter_by(id=check_id, vehicle_id=vehicle_id).first_or_404()
    
    # تسجيل العملية
    log_activity('view', 'VehicleSafetyCheck', safety_check.id, f"عرض تفاصيل فحص السلامة للمركبة: {vehicle.plate_number}")
    
    return render_template('vehicles/safety_check_details.html', vehicle=vehicle, safety_check=safety_check)

@vehicles_bp.route('/<int:vehicle_id>/safety_check_delete/<int:check_id>', methods=['POST'])
@login_required
def delete_safety_check_action(vehicle_id, check_id):
    """حذف فحص سلامة محدد"""
    vehicle = Vehicle.query.get_or_404(vehicle_id)
    safety_check = VehicleSafetyCheck.query.filter_by(id=check_id, vehicle_id=vehicle_id).first_or_404()
    ...
    old_data = {
        'check_date': safety_check.check_date.isoformat() if safety_check.check_date else None,
        'check_type': safety_check.check_type,
        'status': safety_check.status,
        'issues_found': safety_check.issues_found,
        'driver_name': safety_check.driver_name,
        'supervisor_name': safety_check.supervisor_name
    }
    ...
    try:
        ...
        # تسجيل العملية
        log_activity('delete', 'VehicleSafetyCheck', check_id, 
                    f"تم حذف فحص السلامة للمركبة: {vehicle.plate_number}",
                    previous_data=old_data)
        
        flash('تم حذف فحص السلامة بنجاح', 'success')
    except Exception as e:
        db.session.rollback()
        flash('حدث خطأ أثناء حذف فحص السلامة', 'error')
